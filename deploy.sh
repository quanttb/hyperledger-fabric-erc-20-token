...
export TOOLS_VERSION=2.3.1
export PEER_VERSION=2.3.1
export ORDERER_VERSION=2.3.1

# Common functions
function run_ca() {
  docker run -v /tmp/hyperledger:/tmp/hyperledger --network="${COMPOSE_PROJECT_NAME}_net" --rm hyperledger/fabric-ca:${CA_VERSION} sh -c "$*"
  sudo chown -R $(whoami) /tmp/hyperledger
}

function run_tools() {
  docker run -v /tmp/hyperledger:/tmp/hyperledger --network="${COMPOSE_PROJECT_NAME}_net" \
